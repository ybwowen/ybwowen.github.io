...
(async () => {
 ...
 for (const width of [1440,768,375]) {
  ...
  for (const route of routes) {
   ...
   const key = route === '/' ? '' : route.startsWith('/publication') ? 'Publications' : route.startsWith('/talks') ? 'Talks' : route.startsWith('/posts') || route.startsWith('/year-archive') ? 'Blog Posts' : route.startsWith('/portfolio') ? 'Projects' : route.startsWith('/slides') ? 'Slides' : 'CV';
